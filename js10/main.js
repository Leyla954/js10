...
// 2) objectler icinde ne qeder user ,admin ,moderator oldugunu tapin ve array daxilinde loga cixaran func yazin

// const detail = () => {
//     let userCount = users.filter(user=>user.role=="user").length;
//     let adminCount =users.filter(user=>user.role =="admin").length;
//     let moderatorCount = users.filter(user=>user.role=="moderator").length;
//     return[`user:${userCount}`, `admin: ${adminCount}`, `moderator: ${moderatorCount}`]
// }
// console.log(detail());
...
// 3) "department": "Accounting" olanlari secib  array sekilinde ekrana cixaran func yazin

// const getDepartment =()=>users.filter(item => item.company.department=="Accounting")
...
// const getEmail =()=>  users.filter(item => item.email.endsWith("gmail.com"))
